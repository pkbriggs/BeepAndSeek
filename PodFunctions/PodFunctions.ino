...
void blinkTime(int delayms) {
  digitalWrite(ledPin, HIGH);
  analogWrite(speakerOut, 20);
  if (digitalRead(buttonPin) != HIGH) return; //if button is pressed
  delay(delayms);
...
void panicMode() {
  ...
  while (true) {
   blinkTime(DELAY_TIME/4);
   if (digitalRead(buttonPin) == LOW) break;
  }
  //}
 play_rtttl(songs[random(0,NUM_SONGS-1)]); //if repositioned
}
...
void onMode() {
  while (true) {
    ...
    if (buttonState != HIGH) break; //if button is pressed
  }
  play_rtttl(songs[random(0,NUM_SONGS-1)]); //pressed the button. Found the puck!
}
