...
#define RESET 5      //Hub tells pod to turn off, wait for further signals.
    ...
    if (ntimes == -1) { //Send the message forever, until you get a response
      while (true) {
        ...
        if (Serial.available() > 0) {
            ...
            readData(received);
            if (data[1] == OK && data[0] == podID) return true; //Received an acknowledgement.
        }
      }
    }
